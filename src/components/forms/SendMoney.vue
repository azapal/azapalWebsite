...
<template>
  <form @submit.prevent="resumePayment" class="p-3" v-if="!lookedUpBusiness">
    <label for="business"
      ...
      <input type="text" id="Username" v-model="initiatePaymentModel.business_identification_name" placeholder="@businessname" required
    ...
    <label for="amount"
      ...
      <input type="tel" id="amount" v-model="initiatePaymentModel.amount" placeholder="" required
        class="mt-1 w-full border-none bg-transparent p-0 focus:border-transparent focus:ring-0 focus:outline-hidden sm:text-sm dark:text-white" />

    </label>

    <div class="mt-3">
      <p class="text-xs">You must <router-link to="/identity-verification" class="text-blue-500">Add a Bank Information</router-link> before you can send money</p>
      <div class="flex gap-3 mt-3 items-center justify-end w-full">
        ...
        <Button v-else type="submit" variant="outline" v-slot:child>Look up "{{ initiatePaymentModel.business_identification_name }}"</Button>
      </div>
      </div>
    </div>
  </form>
  ...
  <div class="" v-if="lookedUpBusiness">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center">
          <div class="w-10 h-10 rounded-full bg-purple/20 flex items-center justify-center">
            <span class="text-purple font-bold">{{ lookedUpBusiness?.business?.name?.substring(0, 2).toUpperCase() }}</span>
          </div>
          <div class="ml-3">
      ...
      <div class="space-y-3">
        ...
        <div class="flex justify-between items-center">
          ...
          <span class="font-semibold" :class="{'text-orange-500': selectedDeliveryArea}">
            ${{ deliveryFee }}
            <span v-if="!selectedDeliveryArea" class="text-xs text-red-500">(select area)</span>
          </span>
        </div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <Button @click="cancelPayment" class="bg-gray-100 hover:bg-gray-200 py-3 rounded-lg font-medium transition-colors" v-slot:child>
        Cancel
      </Button>
      <Button 
